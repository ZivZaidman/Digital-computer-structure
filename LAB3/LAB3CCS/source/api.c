...
//              state4
//----------------------------------------------------
void BlockSizeStr(char strarr[],char indexarr[]) {
    int i = 0;
    //int countzero = 0;
    int temp = 0;
    int j = 0;
    for (i = 0; i < 100; i++){
        if ((strarr[i] == 0x20)&(i!=0)) {
            indexarr[j] = i-temp;
            temp = i;
            j++;
        }else if (i==99){
...
void state4Func(){
    ...
    for (i=0;i<14;i++){
        stop_timers();
        disable_TimerBCCIE();
        BlockCounter1+=index4[13-i];
        DMA1config();
        startDMA2(&str[100-BlockCounter1],&strSwap[BlockCounter2],index4[13-i]);
        BlockCounter2+=index4[13-i];
    }
    lcd_cmd(0x01);
    lcd_puts(&strSwap[1],30);
    enable_interrupts();
    while (state==state4){
                __bis_SR_register(LPM0_bits + GIE);
                if ((state!=state4)|(KA!=10)){
                    continue;
                }
                lcd_cmd(0x01);
                lcd_puts(&strSwap[31],31);
                while (state==state4){
                    __bis_SR_register(LPM0_bits + GIE);
                    if ((state!=state4)|(KA!=10)){
                        continue;
