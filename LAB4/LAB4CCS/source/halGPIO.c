...
#endif
{
  ...
  {
      if(state_or_set_x && state_or_getString){                     //state mode
          switch(input[0]){
              case '1' :
                  state = state1;
                  break;
      ...
      else if (state_or_set_x == 0){                                   // set x
          x = atoi(input);
          state_or_set_x = 1;
          if (state==state4){
              state=state0;
          }
      }else if ( state_or_getString==0){
          int i=0;
          for (i=0;i<16;i++){
              if (input[i]!='q'){
                  Stringinput[i]=input[i];
                  dontPrintInd=1;
                  if (input[i]==0){
                      Stringinput[i]=0x20;
                  }
      ...
      if (Stringinput[0]!=0x20){
          gobacktosleepind=1;
      }else{gobacktosleepind=0;}
      state_or_getString=1;
      input_slot = 0;
